...
def get_img_post_path(root_dir, split_dir, img_name):
    return os.path.join(root_dir, split_dir, IMG_POST_FOLDER_NAME, img_name)


def get_img_path(root_dir, split_dir, img_name):
    return os.path.join(root_dir, split_dir, IMG_FOLDER_NAME, img_name)


def get_label_path(root_dir, split_dir, img_name):
    return os.path.join(root_dir, split_dir, ANNOT_FOLDER_NAME, img_name.replace('.jpg', label_suffix))
...
class ImageDataset(data.Dataset):
    ...
    def __init__(self, root_dir, split='train', img_size=256, is_train=True,to_tensor=True):
        ...
        self.list_path = os.path.join(self.root_dir, self.split, LIST_FOLDER_NAME, self.split+'.txt') ## CF/LEVIR-CD256/train/list/train.txt
...
class CDDataset(ImageDataset):
    ...
    def __getitem__(self, index):
        ...
        if label.max() > 1:
            label = (label > 127).to(torch.uint8)   # 0/255 -> 0/1
        label = label.unsqueeze(0)

        """print(label.max())
        # --- DEBUG: inspecciona valores únicos en las máscaras ---
        if self.split.lower() == "test":
            # label es un tensor [1,H,W]
            uniques = torch.unique(label)
            print(f"[DEBUG] Split={self.split} | Unique label values: {uniques.tolist()}")"""
