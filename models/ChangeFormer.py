...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        # x: [B, N, C] donde N = H*W a esta altura del stage
        B, N, C = x.shape

        # Q: [B,h,N,d]
        q = self.q(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)

        # K,V: si hay SR se reduce la secuencia
        if self.sr_ratio > 1:
        ...
        # Guardar shapes para profiling
        self._last_shapes = {
            'B': B, 'h': self.num_heads, 'Nq': N, 'Nk': Nk, 'd': d, 'sr_ratio': self.sr_ratio
        }
...
class Attention_dec(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        # expandir task queries a batch si B>1
        task_q = self.task_query
        if B>1:
            
            task_q = task_q.unsqueeze(0).repeat(B,1,1,1)
            task_q = task_q.squeeze(1) # [B, 48, C]

        # Q (sobre task queries): [B,h,Nq,d] con Nq=48 (luego se interpola)
        q = self.q(task_q).reshape(B, task_q.shape[1], self.num_heads, C // self.num_heads).permute(0, 2, 1, 3) # [B,h,48,d]
        ...
        # Ajustar Nq de queries a Nk (longitud espacial de K/V) por interpolación
        # q: [B,h,Nq,d] -> [B,h,Nk,d]
        if q.shape[2] != Nk:
            q = F.interpolate(q, size=(Nk, q.shape[3]), mode='nearest')
        ...
        #x = (attn @ v).transpose(1, 2).reshape(B, N, C)
        x = (attn @ v).transpose(1, 2).reshape(B, Nk, C)

        # Salida final quiere [B, N, C] (N original del caller)
        # Si Nk != N, reinterpolamos a N por el eje secuencia:
        if x.shape[1] != N:
            x = x.transpose(1, 2)                    # [B,C,Nk]
            x = F.interpolate(x, size=N, mode='nearest')
        ...
        # Guardar shapes para profiling
        self._last_shapes = {
            'B': B, 'h': self.num_heads, 'Nq': q.shape[2], 'Nk': Nk, 'd': d, 'sr_ratio': self.sr_ratio
        }
        return x
...
class BlockLinear(nn.Module):
    """
    Igual interfaz que el Block original, pero con atención lineal.
    Usa SR (spatial reduction) sólo para K/V (como en SegFormer/MixFormer).
    """
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=True,
        ...
        self.attn = LinearAttention(dim=dim, heads=num_heads, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x, H, W):
        ...
        if self.sr is not None:
            kv = x1.transpose(1,2).reshape(B, C, H, W)
            kv = self.sr(kv)                  # [B, C, H', W']
        ...
        x = x + self.drop_path(self.attn(q_in, kv))
        x = x + self.drop_path(self.mlp(self.norm2(x), H, W))
        return x
...
class EncoderTransformer_v4(nn.Module):
    """
    Igual a v3 pero con BlockLinear (atención lineal) en cada stage.
    """
    def __init__(self, img_size=256, patch_size=3, in_chans=3, num_classes=2, embed_dims=[64,128,320,512],
                 num_heads=[1,2,4,8], mlp_ratios=[4,4,4,4], qkv_bias=True, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[3,3,4,3], sr_ratios=[8,4,2,1]):
        super().__init__()
        self.num_classes = num_classes
        self.embed_dims = embed_dims
        self.depths = depths
        ...
        self.block1 = nn.ModuleList([
            BlockLinear(dim=embed_dims[0], num_heads=num_heads[0], mlp_ratio=mlp_ratios[0], qkv_bias=qkv_bias,
        ...
        self.block2 = nn.ModuleList([
            BlockLinear(dim=embed_dims[1], num_heads=num_heads[1], mlp_ratio=mlp_ratios[1], qkv_bias=qkv_bias,
                        drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i],
        ...
        self.block3 = nn.ModuleList([
            BlockLinear(dim=embed_dims[2], num_heads=num_heads[2], mlp_ratio=mlp_ratios[2], qkv_bias=qkv_bias,
                        drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i],
        ...
        self.block4 = nn.ModuleList([
            BlockLinear(dim=embed_dims[3], num_heads=num_heads[3], mlp_ratio=mlp_ratios[3], qkv_bias=qkv_bias,
                        drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i],
    ...
    def forward_features(self, x):
        ...
        for blk in self.block1:
            x1 = blk(x1, H, W)
        x1 = self.norm1(x1)
        outs.append(x1.reshape(B, H, W, -1).permute(0,3,1,2).contiguous())

        # stage 2
        x1, H, W = self.patch_embed2(outs[-1])
        ...
        # stage 3
        x1, H, W = self.patch_embed3(outs[-1])
        ...
        # stage 4
        x1, H, W = self.patch_embed4(outs[-1])
...
class ChangeFormerV7(nn.Module):
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256):
        super().__init__()
        self.embed_dims = [64, 128, 320, 512]
        self.depths     = [3, 3, 4, 3]
        ...
        self.Tenc_x2 = EncoderTransformer_v4(
            img_size=256, patch_size=7, in_chans=input_nc, num_classes=output_nc, embed_dims=self.embed_dims,
            num_heads=[1,2,4,8], mlp_ratios=[4,4,4,4], qkv_bias=True, qk_scale=None, drop_rate=self.drop_rate,
        ...
        self.TDec_x2 = DecoderTransformer_v3(
            input_transform='multiple_select', in_index=[0,1,2,3], align_corners=False,
            in_channels=self.embed_dims, embedding_dim=self.embedding_dim, output_nc=output_nc,
            decoder_softmax=decoder_softmax, feature_strides=[2,4,8,16]
        )
